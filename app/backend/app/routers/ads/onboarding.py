...
router = APIRouter(tags=["Ads Panel Onboarding"])
...
async def get_onboarding_checklists(
    ...
    query = select(AdOnboardingChecklist).where(AdOnboardingChecklist.user_id == current_user.id)
    if project_id:
        query = query.where(AdOnboardingChecklist.project_id == project_id)
        
    result = await db.execute(query.order_by(desc(AdOnboardingChecklist.created_at)))
...
async def create_onboarding_checklist(
    ...
    new_item = AdOnboardingChecklist(**in_data.model_dump(), user_id=current_user.id)
    db.add(new_item)
    await db.commit()
...
async def update_onboarding_checklist(
    ...
    result = await db.execute(select(AdOnboardingChecklist).where(
        AdOnboardingChecklist.id == item_id, AdOnboardingChecklist.user_id == current_user.id
    ))
