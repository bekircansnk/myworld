...
router = APIRouter(tags=["Ads Panel AI Observations"])
...
async def get_ai_observations(
    ...
    query = select(AdAiObservation).where(AdAiObservation.user_id == current_user.id)
    if project_id:
        query = query.where(AdAiObservation.project_id == project_id)
        
    result = await db.execute(query.order_by(desc(AdAiObservation.created_at)))
...
async def create_ai_observation(
    ...
    new_item = AdAiObservation(**in_data.model_dump(), user_id=current_user.id)
    db.add(new_item)
    await db.commit()
...
async def update_ai_observation(
    ...
    result = await db.execute(select(AdAiObservation).where(
        AdAiObservation.id == item_id, AdAiObservation.user_id == current_user.id
    ))
