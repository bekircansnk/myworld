...
async def get_reports(
    ...
    query = select(AdReportTemplate).where(AdReportTemplate.user_id == current_user.id)
    if project_id:
        query = query.where(AdReportTemplate.project_id == project_id)
        
    result = await db.execute(query.order_by(desc(AdReportTemplate.created_at)))
...
async def create_report(
    ...
    new_report = AdReportTemplate(**report_in.model_dump(), user_id=current_user.id)
    db.add(new_report)
    await db.commit()
...
async def update_report(
    ...
    result = await db.execute(select(AdReportTemplate).where(
        AdReportTemplate.id == report_id, AdReportTemplate.user_id == current_user.id
    ))
...
async def get_ai_reports(
    ...
    query = select(AdAiAnalysisReport).where(AdAiAnalysisReport.user_id == current_user.id)
    if project_id:
        query = query.where(AdAiAnalysisReport.project_id == project_id)
        
    result = await db.execute(query.order_by(desc(AdAiAnalysisReport.created_at)))
    return result.scalars().all()
...
async def create_ai_analysis(
    ...
    if file and report_source in ["external", "hybrid"]:
        # Sanitize the filename to prevent Path Traversal (CWE-22) vulnerabilities
        file_name = os.path.basename(file.filename) if file.filename else "unnamed"
    ...
    config = {"title": title, "period": "current", "notes": "Generated by Planla AI"}
...
async def get_ai_report(
    ...
    result = await db.execute(select(AdAiAnalysisReport).where(
        AdAiAnalysisReport.id == report_id, AdAiAnalysisReport.user_id == current_user.id
    ))
    report = result.scalar_one_or_none()
    if not report:
