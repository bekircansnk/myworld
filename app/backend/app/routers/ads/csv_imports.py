...
router = APIRouter(tags=["Ads Panel CSV Imports"])
...
async def get_csv_imports(
    ...
    query = select(AdCsvImport).where(AdCsvImport.user_id == current_user.id)
    if project_id:
        query = query.where(AdCsvImport.project_id == project_id)
        
    result = await db.execute(query.order_by(desc(AdCsvImport.created_at)))
...
async def create_csv_import(
    ...
    new_item = AdCsvImport(**in_data.model_dump(), user_id=current_user.id)
    db.add(new_item)
    await db.commit()
...
async def update_csv_import(
    ...
    result = await db.execute(select(AdCsvImport).where(
        AdCsvImport.id == item_id, AdCsvImport.user_id == current_user.id
    ))
