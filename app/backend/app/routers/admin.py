...
async def cleanup_orphan_data(
    ...
    if valid_ids:
        ...
        # Görevler (Optimized: Use rowcount from direct delete to avoid O(N) memory fetch)
        res = await db.execute(delete(Task).where(Task.project_id.isnot(None), Task.project_id.notin_(valid_ids)))
        deleted["tasks"] = res.rowcount
        
        # Notlar
        res = await db.execute(delete(Note).where(Note.project_id.isnot(None), Note.project_id.notin_(valid_ids)))
        deleted["notes"] = res.rowcount
        
        # Takvim
        res = await db.execute(delete(CalendarEvent).where(CalendarEvent.project_id.isnot(None), CalendarEvent.project_id.notin_(valid_ids)))
        deleted["events"] = res.rowcount
        ...
        orphan_session_ids = [r[0] for r in orphan_sessions.all()]
        deleted["sessions"] = len(orphan_session_ids)
        if orphan_session_ids:
            msg_res = await db.execute(delete(ChatMessage).where(ChatMessage.session_id.in_(orphan_session_ids)))
            deleted["messages"] = msg_res.rowcount
