...
export function NotesList() {
  ...
  // Filter Logic
  // ⚡ Bolt Optimization: Memoize filtering and hoist toLowerCase to prevent O(N) recalculations on unrelated keystrokes
  const filteredNotes = React.useMemo(() => {
    ...
    return notes.filter(n => {
      ...
      const cat = (n.ai_category || "").toLowerCase()
      if (activeTab === "Yaratıcı Fikirler") return cat.includes("yaratıcı") || cat.includes("fikir")
      if (activeTab === "Genel Notlar") return cat.includes("genel") || !n.ai_category
      if (activeTab === "Yazılım") return cat.includes("yazılım") || cat.includes("kod")
      return true
    })
