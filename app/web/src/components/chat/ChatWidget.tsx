  ...
  return (
    <div className={`flex w-full ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div className="max-w-[85%] space-y-1">
        ...
        {!isUser && msg.actions && msg.actions.filter(a => !a.action.includes('DETECT_TONE')).length > 0 && (
          <div className="flex flex-wrap gap-1 px-1">
            {msg.actions.filter(a => !a.action.includes('DETECT_TONE')).map((action, i) => (
              <span
                key={i}
          ...
          <CardHeader className="p-3 border-b flex flex-row items-center justify-between space-y-0 shrink-0">
            ...
            <div className="flex items-center gap-0.5">
              ...
              <Button
                ...
                aria-label="Debug modunu aç veya kapat"
              >
                <Bug className="w-3.5 h-3.5" />
              </Button>
              <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-foreground" onClick={clearHistory} aria-label="Geçmişi temizle" title="Geçmişi temizle">
                <Trash2 className="w-3.5 h-3.5" />
              </Button>
              <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-foreground" onClick={toggleChat} aria-label="Sohbeti kapat" title="Sohbeti kapat">
          ...
          <CardFooter className="p-3 border-t shrink-0 flex-col gap-2">
            ...
            <form onSubmit={handleSend} className="flex w-full items-center space-x-2">
              <Input 
                ...
                autoFocus
              />
              <Button type="submit" size="icon" disabled={!inputValue.trim() || isLoading} className="shrink-0 h-9 w-9" aria-label="Mesaj gönder" title="Mesaj gönder">
