        ...
        <Card className="fixed bottom-0 right-0 md:bottom-6 md:right-6 w-full md:w-[380px] lg:w-[420px] h-[80vh] md:h-[600px] max-h-[85vh] shadow-2xl z-50 flex flex-col border border-border animate-in slide-in-from-bottom-5 rounded-b-none md:rounded-b-xl">
          <CardHeader className="p-3 border-b flex flex-row items-center justify-between space-y-0 shrink-0">
            ...
            <div className="flex items-center gap-0.5">
              ...
              <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-foreground" onClick={clearHistory} aria-label="Geçmişi temizle">
                <Trash2 className="w-3.5 h-3.5" />
              </Button>
              <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-foreground" onClick={toggleChat} aria-label="Sohbeti kapat">
          ...
          <CardFooter className="p-3 border-t shrink-0 flex-col gap-2">
            ...
            <form onSubmit={handleSend} className="flex w-full items-center space-x-2">
              <Input 
                ...
                autoFocus
              />
              <Button type="submit" size="icon" disabled={!inputValue.trim() || isLoading} className="shrink-0 h-9 w-9" aria-label="Mesajı gönder">
