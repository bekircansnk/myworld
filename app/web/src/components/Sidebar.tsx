      ...
      <div className={`flex-1 overflow-y-auto py-6 space-y-8 scrollbar-hide ${isExpanded ? 'px-4' : 'px-2'}`}>
        ...
        <div>
           ...
           <div className={`mt-4 ${isExpanded ? 'px-1' : 'flex justify-center'}`}>
             {isExpanded ? <ProjectForm /> : <Button variant="ghost" size="icon" onClick={() => setIsExpanded(true)} aria-label="Firma Ekle" title="Firma Ekle" className="h-10 w-10 shrink-0"><span className="text-lg">+</span></Button>}
           </div>
        </div>
      </div>
