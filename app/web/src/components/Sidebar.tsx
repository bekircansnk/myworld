  ...
  return (
    <div className={`hidden lg:flex transition-all duration-300 ${isExpanded ? 'w-72' : 'w-20'} glass-sidebar flex-col shadow-2xl z-10 relative`}>
      ...
      <button 
        ...
        className="absolute -right-3 top-8 bg-card border border-border shadow-sm rounded-full p-1 z-50 hover:bg-muted transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
      >
      ...
      <div className={`flex-1 overflow-y-auto py-6 space-y-8 scrollbar-hide ${isExpanded ? 'px-4' : 'px-2'}`}>
        ...
        <div>
             ...
             {mounted && projects.map(proj => (
                 <div key={proj.id} title={proj.name} className={`w-full flex items-center justify-between rounded-xl transition-all duration-300 group btn-3d ${isExpanded ? 'px-2 py-1 text-left text-sm' : 'justify-center p-2'} ${selectedProjectId === proj.id ? 'bg-card shadow-md inner-shadow-box ring-1 ring-border' : 'hover:bg-muted border-transparent'}`}>
                  <button 
                    ...
                    title={`${proj.name} firmasını seç`}
                    className={`flex items-center truncate focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset rounded-lg ${isExpanded ? 'flex-1 gap-3 py-2 px-2' : 'justify-center w-full h-8'}`}
                  >
                  ...
                  {isExpanded && (
                     <button 
                       ...
                       title={`${proj.name} ayarları`}
                       className={`p-2 rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${selectedProjectId === proj.id ? 'opacity-100 3d-button' : 'opacity-0 group-hover:opacity-100'}`}
                     >
           ...
           <div className={`mt-4 ${isExpanded ? 'px-1' : 'flex justify-center'}`}>
             {isExpanded ? <ProjectForm /> : <Button aria-label="Yeni firma ekle" title="Yeni firma ekle" variant="ghost" size="icon" onClick={() => setIsExpanded(true)} className="h-10 w-10 shrink-0"><span className="text-lg">+</span></Button>}
           </div>
        </div>
      </div>
