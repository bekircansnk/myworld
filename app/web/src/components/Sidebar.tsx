...
export function Sidebar() {
  ...
  return (
    <div className={`hidden lg:flex transition-all duration-300 ${isExpanded ? 'w-72' : 'w-20'} glass-sidebar flex-col shadow-2xl z-10 relative`}>
      ...
      >
        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`transition-transform duration-300 ${isExpanded ? '' : 'rotate-180'}`}><path d="m15 18-6-6 6-6"/></svg>
      </button>
      ...
      <div className={`flex-1 overflow-y-auto py-6 space-y-8 scrollbar-hide ${isExpanded ? 'px-4' : 'px-2'}`}>
        ...
        <div>
           ...
           <div className={`mt-4 ${isExpanded ? 'px-1' : 'flex justify-center'}`}>
             {isExpanded ? <ProjectForm /> : <Button variant="ghost" size="icon" onClick={() => setIsExpanded(true)} aria-label="Firma Ekle" className="h-10 w-10 shrink-0"><span aria-hidden="true" className="text-lg">+</span></Button>}
           </div>
        </div>
      </div>
      ...
      <div className={`p-4 border-t border-border bg-muted/20 ${!isExpanded ? 'flex justify-center' : ''}`}>
        {isExpanded ? (
          ...
          >
            {theme === 'dark' ? <Sun className="w-4 h-4 text-amber-500" aria-hidden="true" /> : <Moon className="w-4 h-4 text-indigo-400" aria-hidden="true" />}
          </Button>
        )}
      </div>
