...
function ChecklistForm({ onClose, projectId }: ChecklistFormProps) {
  const { createChecklist, fetchChecklists } = useAdsStore();
...
export function OnboardingChecklist({ projectId }: { projectId: number | null }) {
  const { checklists, isLoadingChecklists, fetchChecklists, updateChecklist, deleteChecklist } = useAdsStore();
  ...
  const toggleItem = (checklist: AdOnboardingChecklist, index: number) => {
