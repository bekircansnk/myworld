...
interface TaskFormProps {
  ...
  initial?: AdAdsTask | null;
}

function TaskForm({ onClose, projectId, initial }: TaskFormProps) {
  const { createTask, updateTask, fetchTasks, campaigns, experiments, creatives, fetchCampaigns, fetchExperiments, fetchCreatives, getAITaskNotes } = useAdsStore();
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const payload: Partial<AdAdsTask> = {
        title,
...
export function AdsTaskBoard({ projectId }: { projectId: number | null }) {
  const { adsTasks, isLoadingTasks, fetchTasks, updateTask, deleteTask, setViewMode, setSelectedEntityToView, selectedEntityToView } = useAdsStore();
  ...
  const handleMoveTask = async (task: AdAdsTask, newStatus: string) => {
                ...
                <div className="p-3 space-y-3 flex-1 min-h-[200px]">
                  {tasks.length === 0 ? (
                    ...
                    tasks.map(task => {
                      ...
                      const { campaigns, experiments, creatives } = useAdsStore.getState();
