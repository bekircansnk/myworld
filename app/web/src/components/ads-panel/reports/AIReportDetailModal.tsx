...
export function AIReportDetailModal({ reportId, onClose, autoDownload = false }: AIReportDetailModalProps) {
  const { isAuthenticated, checkAuth } = useAuthStore();
  
  const [report, setReport] = useState<AdAIAnalysisReport | null>(null);
  ...
  const fetchReport = async (id: string) => {
    try {
      setLoading(true);
      const res = await api.get(`/api/ads/reports/ai-analysis/${id}`);
      setReport(res.data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  if (!isAuthenticated || loading) {
    return (
      <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="flex flex-col bg-slate-50 dark:bg-[#0f1117] rounded-3xl p-12 items-center justify-center shadow-2xl">
          ...
          <p className="mt-4 text-slate-500 font-medium">Rapor Yükleniyor...</p>
        </div>
      </div>
    );
  }
  ...
  if (!report || !report.analysis_result) {
    return (
      <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="flex flex-col bg-slate-50 dark:bg-[#0f1117] rounded-3xl p-12 items-center justify-center shadow-2xl relative">
          <button onClick={onClose} className="absolute top-4 right-4 p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-xl transition-colors">
            <X className="w-6 h-6 text-slate-500 dark:text-slate-400" />
          </button>
          <div className="text-center space-y-4">
            <AlertTriangle className="w-12 h-12 text-amber-500 mx-auto" />
            <h2 className="text-xl font-bold dark:text-white">Rapor Bulunamadı</h2>
            <button onClick={onClose} className="px-5 py-2.5 bg-brand-dark text-white rounded-xl mt-4">Kapat</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-0 md:p-4 print:p-0 print:block print:bg-white print:relative" onClick={onClose}>
      <div className="bg-slate-50 dark:bg-[#0a0c10] w-full h-full md:rounded-3xl md:h-[95vh] md:max-w-7xl flex flex-col overflow-hidden shadow-2xl print:shadow-none print:h-auto print:overflow-visible print:block print:w-full" onClick={e => e.stopPropagation()}>
        
        {/* Modal Header */}
        <div className="flex items-center justify-between px-4 py-3 md:px-6 md:py-4 border-b border-slate-200 dark:border-white/5 bg-white dark:bg-[#0f1117] print:hidden shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-bold text-brand-dark dark:text-white flex items-center gap-2">
              <BarChart3 className="w-5 h-5 text-indigo-500" />
              Yapay Zeka Analiz Detayı
            </h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6 lg:px-8 xl:px-12 print:overflow-visible print:p-0 print:block relative">
