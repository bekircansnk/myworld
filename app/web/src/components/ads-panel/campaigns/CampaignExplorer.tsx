...
export function CampaignExplorer({ projectId }: CampaignExplorerProps) {
  const { campaigns, isLoadingCampaigns, deleteCampaign, selectedEntityToView, setSelectedEntityToView } = useAdsStore();
  ...
  // Optimization: Memoize filtered campaigns to prevent O(N) recalculations on every render.
  // Additionally, hoist `.toLowerCase()` outside the filter loop to avoid redundant string allocations.
  // This significantly reduces main thread blocking when filtering large lists during fast typing.
