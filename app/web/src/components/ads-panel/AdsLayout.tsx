...
import { useAdsStore, AdsViewMode } from '@/stores/adsStore';
...
import { AdsOverview } from './overview/AdsOverview';
...
export function AdsLayout({ projectId }: AdsLayoutProps) {
  const { viewMode, setViewMode, fetchCampaigns } = useAdsStore();
  ...
  const menuItems: { id: AdsViewMode; label: string; icon: React.FC<any> }[] = [
  ...
  return (
    <div className="flex-1 flex flex-col md:flex-row overflow-hidden mobile-content-area">
      ...
      <div className="flex-1 flex flex-col overflow-y-auto overflow-x-hidden p-3 md:p-5 lg:p-8 bg-slate-50/50 dark:bg-[#0f1117]/30">
        {viewMode === 'overview' && <AdsOverview projectId={projectId} />}
