...
export function CreativeForm({ onClose, projectId, initialData }: CreativeFormProps) {
  const { createCreative, updateCreative, campaigns, experiments, fetchCampaigns, fetchExperiments } = useAdsStore();
  ...
  const [formData, setFormData] = useState<Partial<AdCreative>>(
    initialData || {
      creative_name: '',
      creative_type: 'image',
      format: '1x1',
