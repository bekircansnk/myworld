...
export function TestCenter({ projectId }: TestCenterProps) {
  const { experiments, isLoadingExperiments, fetchExperiments, deleteCampaign: deleteExperiment, selectedEntityToView, setSelectedEntityToView, setViewMode } = useAdsStore();
  const { projects } = useProjectStore();
  const currentProject = projects.find(p => p.id === projectId);
  const { campaigns, creatives } = useAdsStore();
  ...
  const handleOpenEdit = (test: AdExperiment) => {
    setEditingTest(test);
    setIsFormOpen(true);
  };
