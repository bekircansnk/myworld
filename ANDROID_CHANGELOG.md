...
- **[UX - Hızlı Ses Çıkış Yönlendirmesi]** Ayarlar çarkının altına gizlenen kulağım (kulaklık) ve karşı taraf (hoparlör) çıkış kanalı seçicileri doğrudan ana ekrana çıkarılarak "Hızlı Ses Geçiş Modu" paneli oluşturuldu.
- **[UX - Tam Ekran Mobil Transkript]** Mobilde yazı geçmişi transkript modalı, daha rahat okunabilmesi için tam ekran (`h-[92vh]`) bottom-sheet baloncuğuna dönüştürüldü.
- **[Android Arayüz Düzeltmesi]** Android WebView'da ekranın sağa kayması ve taşması, esnek CSS flex-box düzeni kurularak kökten çözüldü.
- **[Sürüm Yükseltme]** `versionCode 58` ve `versionName "6.8"` debug APK derlemesi tamamlandı.
...
- **[WebSocket Kararlılığı]** Bağlantı kopmaları ve geçersiz API key durumlarındaki sonsuz reconnect döngüsü kırıldı. Hata nedenleri doğrudan durum çubuğunda gösterilecek şekilde loglandı.
- **[Sürüm Yükseltme]** `versionCode 57` ve `versionName "6.7"` debug APK derlemesi yapıldı.
...
**Tarih:** 15 Haziran 2026
**Değişiklikler:**
- **[Yeni Özellik - Akıllı Ses Yönlendirme (Smart Audio Routing)]** Kulaklık takılıyken Türkçe seslerin İngilizceye çevrilip **Dış Hoparlörden** çalması, İngilizce seslerin ise Türkçeye çevrilip sadece **Kulaklıktan** çalması sağlandı (`AudioContext.setSinkId` entegrasyonu).
- **[Yeni Özellik - Otomatik Çeviri Modu (Continuous Mode)]** Sürekli dinleme (VAD) ve otomatik çift yönlü dil algılama yeteneği kazandırıldı.
- **[Masaüstü Entegrasyonu]** Sesli Çeviri modülü üst navigasyon menüsüne eklenerek masaüstü web tarayıcılarında da kullanılabilir hale getirildi.
- **[Sürüm Yükseltme]** `versionCode 56` ve `versionName "6.6"` debug APK derlemesi yapıldı.
...
- **[Tema Birleştirmesi]** `AppTheme` ve `AppTheme.NoActionBar` temaları tek bir `NoActionBar` tabanlı tema altında birleştirildi. `windowActionBar: false`, `windowNoTitle: true`, `windowBackground`, `statusBarColor` ve `navigationBarColor` tümü ana temada tanımlandı.
- **[Sürüm Yükseltme]** `versionCode 54` ve `versionName "6.4"` debug APK derlemesi yapıldı.
...
**Tarih:** 8 Haziran 2026
**Değişiklikler:**
- **[Yeni Özellik - Manuel Güncelleme Kontrolü]** Kullanıcı profil menüsüne "Güncellemeleri Denetle" butonu eklendi.
- **[Yeni Özellik - Güncelleme Baloncuğu & Göstergesi]** Güncelleme uyarısı kapatıldığında (dismiss) kaybolmaması için ekranın sağ altında floating güncelleme baloncuğu eklendi. Profil butonu üzerinde güncelleme durumunda yanan kırmızı nokta bildirim göstergesi eklendi.
- **[Kritik Görsel - Splash Screen Sızması & ActionBar Giderildi]** Android `styles.xml` temasında `postSplashScreenTheme` tanımlanarak splash screen geçişi stabil hale getirildi. `windowBackground` ayarlanarak arkadaki splash resminin sızması kesin olarak çözüldü.
- **[Sürüm Yükseltme]** `versionCode 53` ve `versionName "6.3"` debug APK derlemesi yapıldı.
