...
- `MainActivity` içinde `WindowCompat.setDecorFitsSystemWindows(getWindow(), true)` kullanılır.
- Android tema tarafında status/navigation bar renkleri sabitlenir.
- Mobil tarayıcı/PWA düzeni bozulmaması için web'e genel ekstra üst padding verilmez; düzeltme native katmanda yapılır.
...
**Çözüm:**
- Dashboard üst özet alanı `xl` öncesinde dikey, kontrollü grid düzeninde akar; h1 ve metrik fontları küçültülüp `leading` değerleri sabitlendi.
- Mobil metrikler 4 kolonlu, ortalı ve kısa satır yüksekliğinde kalır; ilerleme barı küçük telefonda ikinci satıra iner.
- TopNavbar safe-area hesabı tüm platformlarda `env(safe-area-inset-top)+0.5rem` olarak sadeleştirildi; aksiyon ikonları 44px civarı dokunma hedefiyle ortalandı.
...
- Sütun içi `touch-pan-y` sınıfı kaldırıldı; dikey kaydırma `overflow-y-auto`, momentum scroll ve `overscroll-behavior-y: contain` ile korunur.
- Mobilde `scroll-snap-type: x mandatory` yerine `x proximity` kullanılır. Böylece yatay hareket başlatma hissi sert kilitlenmez.
- Kural: nested scroll alanlarında tek eksenli `touch-pan-x` veya `touch-pan-y` kullanılacaksa, aynı dokunuş hedefinden diğer eksenin gerekli olmadığı kanıtlanmadan eklenmemelidir.
...
- Sütun içi scroll alanlarında tek eksenli `touch-pan-y` kullanılmamalıdır; aksi durumda yatay board kaydırma kart/sütun hedefinden başlayamaz.
...
## 📡 Gemini Live API (WebSocket) Sorunları

### 1. Canlı Çeviri WebSocket Bağlantısının Kurulamaması ve Reconnect Döngüsü (Hata 1008)
**Sorun:** Çeviriyi başlat dedikten sonra durumun "Bağlantı kuruluyor" ile "Canlı bağlantı aktif" arasında sürekli gidip gelmesi ve hiçbir sesli veya yazılı çeviri gerçekleşmemesi.
**Neden:** Google AI Studio, Gemini 2.0 deneysel modellerinin exp sürümlerini (`models/gemini-2.0-flash-exp`) yayından kaldırmıştır. WebSocket endpoint'i (`v1alpha` veya `v1beta`) setup payload'unda bu modeli aradığında bulamayıp WebSocket bağlantısını `Code: 1008` (Policy/Argument Violation) ile derhal sonlandırıyordu. Uygulama da otomatik olarak diğer keye geçip tekrar denediği için bu durum sonsuz reconnect döngüsüne yol açıyordu.
**Çözüm:**
- `useLiveTranslate.ts` dosyasındaki setup payload'undaki model adı en güncel stabil sürüm olan `"models/gemini-2.5-flash-native-audio-latest"` olarak güncellendi.
- API anahtarlarının geçerliliği Curl ve Node scriptleri ile test edilerek her iki keyin de bu modelle WebSocket handshake'ini 200 OK ile geçtiği doğrulandı.
- Telefondaki veya bilgisayardaki hataları daha rahat takip edebilmek için ekranın altına açılıp kapatılabilir bir "Canlı Bağlantı Logları ve Teşhis Konsolu" entegre edildi. Bu konsol tüm websocket olaylarını, close kodlarını ve key rotasyonlarını anlık olarak gösterir.
- Android WebView'da ekran taşmaları ve kaymalarını önlemek için dikey scrollbox (`overflow-y-auto` ve `pb-24`) yapısı kurularak mobil uyumluluk sağlandı.
- Hızlı geçişler için kulaklık/hoparlör çıkış aygıtı seçicileri doğrudan ana ekrana "Hızlı Ses Geçiş Modu" paneliyle yerleştirildi.
- Mobil cihazlarda yazı geçmişi transkript modalı, rahat okunabilmesi için tam ekran bottom-sheet baloncuğuna dönüştürüldü.
